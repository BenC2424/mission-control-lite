...
export async function assignTask(taskId, agentId) {
  ...
  await backend.run(
    sql({
      ...
      postgres: `INSERT INTO task_assignments (task_id, agent_id, assigned_at, tenant_id)
                 VALUES ($1, $2, $3, $4)
                 ON CONFLICT (task_id, agent_id)
                 DO UPDATE SET assigned_at = EXCLUDED.assigned_at, claimed_at = NULL, completed_at = NULL, seen_at = NULL`
    }),
    taskId,
    agentId,
    ts,
  ...
  if (backend.mode === 'sqlite') {
    await backend.run(
      'UPDATE task_assignments SET assigned_at=?, claimed_at=NULL, completed_at=NULL, seen_at=NULL WHERE tenant_id=? AND task_id=? AND agent_id=?',
      ts,
      TENANT_ID,
      taskId,
      agentId
...
export async function reassignTask(taskId, agentId) {
  const ts = now();
  const begin = backend.mode === 'postgres' ? 'BEGIN' : 'BEGIN IMMEDIATE';
  const openBefore = await backend.get(
    sql({
      sqlite: 'SELECT COUNT(*) as c FROM task_assignments WHERE tenant_id=? AND task_id=? AND completed_at IS NULL',
      postgres: 'SELECT COUNT(*)::int as c FROM task_assignments WHERE tenant_id=$1 AND task_id=$2 AND completed_at IS NULL'
    }),
    TENANT_ID,
    taskId
  );
  const targetBefore = await backend.get(
    sql({
      sqlite: 'SELECT COUNT(*) as c FROM task_assignments WHERE tenant_id=? AND task_id=? AND agent_id=?',
      postgres: 'SELECT COUNT(*)::int as c FROM task_assignments WHERE tenant_id=$1 AND task_id=$2 AND agent_id=$3'
    }),
    TENANT_ID,
    taskId,
    agentId
  ...
  try {
    ...
    await backend.run(
      sql({
        sqlite: 'UPDATE task_assignments SET completed_at = COALESCE(completed_at, ?) WHERE tenant_id = ? AND task_id = ? AND completed_at IS NULL',
        postgres: 'UPDATE task_assignments SET completed_at = COALESCE(completed_at, $1) WHERE tenant_id = $2 AND task_id = $3 AND completed_at IS NULL'
      }),
      ts,
      TENANT_ID,
      taskId
    ...
    if (await hasLastActivityColumn()) {
      await backend.run(
        sql({
          sqlite: 'UPDATE tasks SET owner=?, status=?, updated_at=?, last_activity_at=? WHERE id=? AND tenant_id = ?',
          postgres: 'UPDATE tasks SET owner=$1, status=$2, updated_at=$3, last_activity_at=$4 WHERE id=$5 AND tenant_id = $6'
        }),
        agentId,
        'assigned',
        ts,
      ...
      await backend.run(
        sql({
          sqlite: 'UPDATE tasks SET owner=?, status=?, updated_at=? WHERE id=? AND tenant_id = ?',
          postgres: 'UPDATE tasks SET owner=$1, status=$2, updated_at=$3 WHERE id=$4 AND tenant_id = $5'
        }),
        agentId,
        'assigned',
        ts,
    ...
    const openAfter = await backend.get(
      sql({
        sqlite: 'SELECT COUNT(*) as c FROM task_assignments WHERE tenant_id=? AND task_id=? AND completed_at IS NULL',
        postgres: 'SELECT COUNT(*)::int as c FROM task_assignments WHERE tenant_id=$1 AND task_id=$2 AND completed_at IS NULL'
      }),
    ...
    const targetAfter = await backend.get(
      sql({
        sqlite: 'SELECT COUNT(*) as c FROM task_assignments WHERE tenant_id=? AND task_id=? AND agent_id=? AND completed_at IS NULL',
        postgres: 'SELECT COUNT(*)::int as c FROM task_assignments WHERE tenant_id=$1 AND task_id=$2 AND agent_id=$3 AND completed_at IS NULL'
      }),
      TENANT_ID,
      taskId,
      agentId
    ...
    return {
      taskId,
      agentId,
      open_before: Number(openBefore?.c || 0),
      target_before: Number(targetBefore?.c || 0),
      open_after: Number(openAfter?.c || 0),
      target_after: Number(targetAfter?.c || 0),
      commit_success: true
    };
  } catch (err) {
    try { await backend.exec('ROLLBACK'); } catch {}
    return { taskId, agentId, commit_success: false, error: String(err?.message || err) };
  }
}
...
export async function getTaskAssignmentsDebug(taskIds = []) {
  const ids = Array.from(new Set((taskIds || []).map((x) => String(x || '').trim()).filter(Boolean)));
  if (!ids.length) return [];

  if (backend.mode === 'postgres') {
    const rows = await backend.all(
      `SELECT task_id, agent_id, assigned_at, claimed_at, completed_at
       FROM task_assignments
       WHERE tenant_id = $1 AND task_id = ANY($2::text[])
       ORDER BY task_id, agent_id, assigned_at`,
      TENANT_ID,
      ids
    );
    return rows.map((r) => ({ task_id: r.task_id, agent_id: r.agent_id, assigned_at: r.assigned_at, claimed_at: r.claimed_at, completed_at: r.completed_at }));
  }

  const placeholders = ids.map(() => '?').join(',');
  const sqlText = `SELECT task_id, agent_id, assigned_at, claimed_at, completed_at
                   FROM task_assignments
                   WHERE tenant_id = ? AND task_id IN (${placeholders})
                   ORDER BY task_id, agent_id, assigned_at`;
  const rows = await backend.all(sqlText, TENANT_ID, ...ids);
  return rows.map((r) => ({ task_id: r.task_id, agent_id: r.agent_id, assigned_at: r.assigned_at, claimed_at: r.claimed_at, completed_at: r.completed_at }));
}

export async function getTaskCoreDebug(taskIds = []) {
  const ids = Array.from(new Set((taskIds || []).map((x) => String(x || '').trim()).filter(Boolean)));
  if (!ids.length) return [];

  if (backend.mode === 'postgres') {
    const rows = await backend.all(
      `SELECT id, owner, status, title FROM tasks WHERE tenant_id = $1 AND id = ANY($2::text[]) ORDER BY id`,
      TENANT_ID,
      ids
    );
    return rows.map((r) => ({ id: r.id, owner: r.owner, status: r.status, intent: (String(r.title || '').match(/^\s*\[([^\]]+)\]/)?.[1] || null) }));
  }

  const placeholders = ids.map(() => '?').join(',');
  const sqlText = `SELECT id, owner, status, title FROM tasks WHERE tenant_id = ? AND id IN (${placeholders}) ORDER BY id`;
  const rows = await backend.all(sqlText, TENANT_ID, ...ids);
  return rows.map((r) => ({ id: r.id, owner: r.owner, status: r.status, intent: (String(r.title || '').match(/^\s*\[([^\]]+)\]/)?.[1] || null) }));
}
  ...
  const nextQ = sql({
    sqlite: `
      ...
      WHERE a.tenant_id = ?
        ...
        AND t.title LIKE '[EXEC]%'
        AND t.status NOT IN ('review','done','archived','blocked','in_progress','inbox')
        AND (
...
export async function getMetrics() {
  ...
  const registeredAgents = await backend.all(sql({
    sqlite: `SELECT id FROM agents WHERE tenant_id = ?`,
    postgres: `SELECT id FROM agents WHERE tenant_id = $1`
  }), TENANT_ID);
  const eligibleAgentIds = new Set((registeredAgents || []).map((a) => a.id));
  const TEN_MIN_MS = 10 * 60 * 1000;
  const nowMs = Date.now();
  const agentsHealthy = [...latestByAgent.values()]
    .filter((h) => eligibleAgentIds.has(h?.agentId))
    .filter((h) => h?.status === 'ok')
    .filter((h) => h?.at && (nowMs - new Date(h.at).getTime()) < TEN_MIN_MS)
    .length;
