...
export function updateTask(patch) {
  ...
  if (status === 'done') {
    db.prepare('UPDATE task_assignments SET completed_at = COALESCE(completed_at, ?) WHERE task_id = ?').run(updatedAt, patch.id);
  }
...
export function claimNext(agentId) {
  const next = db.prepare(`
    ...
    LIMIT 1
  `).get(agentId, CLAIM_TIMEOUT_MINUTES);
  if (!next) return null;
...
export function getEscalations(limit = 50) {
  ...
  const staleClaims = db.prepare(`
    ...
    LIMIT ?
  `).all(CLAIM_TIMEOUT_MINUTES, limit);
...
export function getMetrics() {
  ...
  const escalations = getEscalations(200);
  return {
    tasks: counts,
