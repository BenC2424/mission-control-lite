    ...
    backend.get(
      sql({
        ...
        postgres: `SELECT
          ...
          COALESCE(SUM(CASE WHEN status = 'assigned' AND COALESCE(last_activity_at, updated_at, created_at) < $1 THEN 1 ELSE 0 END),0)::int as assigned_gt_24h,
          COALESCE(SUM(CASE WHEN status = 'in_progress' AND COALESCE(last_activity_at, updated_at, created_at) < $2 THEN 1 ELSE 0 END),0)::int as in_progress_gt_8h,
          COALESCE(SUM(CASE WHEN status = 'review' AND COALESCE(last_activity_at, updated_at, created_at) < $3 THEN 1 ELSE 0 END),0)::int as review_gt_12h,
          COALESCE(SUM(CASE WHEN status = 'done' AND COALESCE(updated_at, created_at) >= $4 THEN 1 ELSE 0 END),0)::int as tasks_completed_24h
