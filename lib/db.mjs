...
export async function upsertSubscription({ tenantId, planKey, status = 'trial', billingProvider = 'manual', providerCustomerId = null, providerSubscriptionId = null, currentPeriodStart = null, currentPeriodEnd = null }) {
  ...
  if (backend.mode === 'postgres') {
    await backend.run(
      `INSERT INTO subscriptions (id, tenant_id, plan_key, billing_provider, provider_customer_id, provider_subscription_id, status, current_period_start, current_period_end, created_at, updated_at)
      ...
      id, tenantId, planKey, billingProvider, providerCustomerId, providerSubscriptionId, status, currentPeriodStart, currentPeriodEnd, ts, ts
    );
  } else {
    await backend.run(
      'INSERT OR REPLACE INTO subscriptions (id, tenant_id, plan_key, billing_provider, provider_customer_id, provider_subscription_id, status, current_period_start, current_period_end, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      id, tenantId, planKey, billingProvider, providerCustomerId, providerSubscriptionId, status, currentPeriodStart, currentPeriodEnd, ts, ts
    );
  }
