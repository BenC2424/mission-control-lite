...
export async function listTasks() {
  ...
  const evidenceRows = await backend.all(sql({
    sqlite: `SELECT task_id as taskId, MAX(created_at) as lastExecutionEvidenceAt
             FROM task_events
             WHERE task_id IS NOT NULL AND type IN ('worker_first_progress','worker_progress_heartbeat','task_started','task_note','worker_run_ended')
             GROUP BY task_id`,
    postgres: `SELECT task_id as "taskId", MAX(created_at) as "lastExecutionEvidenceAt"
               FROM task_events
               WHERE task_id IS NOT NULL AND type IN ('worker_first_progress','worker_progress_heartbeat','task_started','task_note','worker_run_ended')
               GROUP BY task_id`
  }));
  const evidenceByTask = new Map(evidenceRows.map((r) => [r.taskId, r.lastExecutionEvidenceAt]));

  // Keep task list endpoint lightweight for board rendering.
  // Per-task notes are loaded on-demand via task note actions and events.
  return tasks.map((t) => ({
    id: t.id,
    title: t.title,
    status: t.status,
    ...
    lastExecutionEvidenceAt: evidenceByTask.get(t.id) || null,
    notes: []
  }));
}
...
export async function resetAssignmentClaim(taskId, agentId) {
  await backend.run(
    sql({
      sqlite: 'UPDATE task_assignments SET claimed_at = NULL WHERE task_id=? AND agent_id=? AND completed_at IS NULL',
      postgres: 'UPDATE task_assignments SET claimed_at = NULL WHERE task_id=$1 AND agent_id=$2 AND completed_at IS NULL'
    }),
    taskId,
    agentId
  );
...
export async function openExecutionLease(taskId, runId, agentId) {
  await backend.run(
    sql({
      sqlite: `INSERT INTO execution_leases (task_id, run_id, agent_id, status, opened_at, closed_at)
               VALUES (?, ?, ?, 'active', ?, NULL)
               ON CONFLICT(task_id) DO UPDATE SET run_id=excluded.run_id, agent_id=excluded.agent_id, status='active', opened_at=excluded.opened_at, closed_at=NULL`,
      postgres: `INSERT INTO execution_leases (task_id, run_id, agent_id, status, opened_at, closed_at)
...
export async function getActiveExecutionLease(taskId) {
  return backend.get(
    sql({
      sqlite: `SELECT task_id as taskId, run_id as runId, agent_id as agentId, status, opened_at as openedAt, closed_at as closedAt
               FROM execution_leases WHERE task_id=? AND status='active'`,
      postgres: `SELECT task_id as "taskId", run_id as "runId", agent_id as "agentId", status, opened_at as "openedAt", closed_at as "closedAt"
                 FROM execution_leases WHERE task_id=$1 AND status='active'`
    }),
    taskId
  );
}

export async function closeExecutionLease(taskId, reason = 'closed') {
  await backend.run(
    sql({
      sqlite: `UPDATE execution_leases SET status=?, closed_at=? WHERE task_id=? AND status='active'`,
      postgres: `UPDATE execution_leases SET status=$1, closed_at=$2 WHERE task_id=$3 AND status='active'`
    }),
    reason,
    now(),
    taskId
...
export async function getEscalations(limit = 50) {
  ...
  const inProgress = await backend.all(sql({
    sqlite: `SELECT id as taskId, title, status, owner, updated_at as updatedAt FROM tasks WHERE status='in_progress'`,
    postgres: `SELECT id as "taskId", title, status, owner, updated_at as "updatedAt" FROM tasks WHERE status='in_progress'`
  }));
  const staleWip = inProgress
    .map((t) => {
      const ts = Date.parse(evidenceByTask.get(t.taskId) || t.updatedAt || '');
      if (!Number.isFinite(ts)) return null;
      const staleMs = Date.now() - ts;
      if (staleMs < (WIP_STALE_MINUTES * 60 * 1000)) return null;
      return {
        taskId: t.taskId,
        title: t.title,
        status: t.status,
        ...
        reason: 'wip_no_evidence_stale'
      };
    })
    .filter(Boolean)
    .slice(0, limit);

  return [...blocked, ...staleClaims, ...staleWip].slice(0, limit);
}
...
export async function getMetrics() {
  ...
  const staleWipCount = escalations.filter((e) => e.reason === 'wip_no_evidence_stale').length;
  return {
    tasks: counts,
...
export async function clearAllData() {
  await backend.exec('DELETE FROM execution_leases; DELETE FROM task_assignments; DELETE FROM task_notes; DELETE FROM tasks; DELETE FROM task_events;');
}
