...
export async function listTasksBoard() {
  const tasks = await backend.all(
    sql({
      sqlite: `SELECT id, title, status, priority, owner, created_at, updated_at FROM tasks
               WHERE tenant_id = ? AND status IN ('inbox','assigned','in_progress','review','done')
               ORDER BY updated_at DESC`,
      postgres: `SELECT id, title, status, priority, owner, created_at, updated_at FROM tasks
                 WHERE tenant_id = $1 AND status IN ('inbox','assigned','in_progress','review','done')
                 ORDER BY updated_at DESC`
    }),
    TENANT_ID
  );
...
export async function hasWorkerStartAck(taskId, agentId) {
  const row = await backend.get(
    sql({
      sqlite: "SELECT 1 as ok FROM task_notes WHERE tenant_id = ? AND task_id = ? AND actor = ? AND note LIKE 'worker_ack_start:%' ORDER BY id DESC LIMIT 1",
      postgres: "SELECT 1 as ok FROM task_notes WHERE tenant_id = $1 AND task_id = $2 AND actor = $3 AND note LIKE 'worker_ack_start:%' ORDER BY id DESC LIMIT 1"
    }),
    TENANT_ID,
    taskId,
    agentId
...
export async function getBoardHealthAggregates(tenantId = TENANT_ID) {
  ...
  const [statusRows, ownerRows, summary, teamRows] = await Promise.all([
    backend.all(
      sql({
    ...
    backend.get(
      sql({
        ...
        postgres: `SELECT
          ...
          COALESCE(SUM(CASE WHEN status = 'in_progress' AND COALESCE(NULLIF(last_activity_at::text,'')::timestamptz, NULLIF(updated_at::text,'')::timestamptz, NULLIF(created_at::text,'')::timestamptz) < $2::timestamptz THEN 1 ELSE 0 END),0)::int as in_progress_gt_8h,
          COALESCE(SUM(CASE WHEN status = 'review' AND COALESCE(NULLIF(last_activity_at::text,'')::timestamptz, NULLIF(updated_at::text,'')::timestamptz, NULLIF(created_at::text,'')::timestamptz) < $3::timestamptz THEN 1 ELSE 0 END),0)::int as review_gt_12h,
          COALESCE(SUM(CASE WHEN status = 'done' AND COALESCE(updated_at::timestamptz, created_at::timestamptz) >= $4::timestamptz THEN 1 ELSE 0 END),0)::int as tasks_completed_24h
          FROM tasks WHERE tenant_id = $5`
      }),
      assignedCutoff,
      inProgressCutoff,
      reviewCutoff,
