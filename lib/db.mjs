...
export async function createTask(task) {
  ...
  if (await hasLastActivityColumn()) {
    const q = sql({
      sqlite: 'INSERT INTO tasks (id, title, status, priority, owner, created_at, updated_at, last_activity_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      postgres: 'INSERT INTO tasks (id, title, status, priority, owner, created_at, updated_at, last_activity_at) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)'
    });
    await backend.run(q, task.id, task.title, task.status, task.priority, task.owner, task.createdAt, ts, ts);
    return;
  }
  ...
  await backend.run(q, task.id, task.title, task.status, task.priority, task.owner, task.createdAt, ts);
}
...
export async function updateTask(patch) {
  ...
  if (await hasLastActivityColumn()) {
    const upd = sql({
      sqlite: 'UPDATE tasks SET status=?, owner=?, priority=?, updated_at=?, last_activity_at=? WHERE id=?',
      postgres: 'UPDATE tasks SET status=$1, owner=$2, priority=$3, updated_at=$4, last_activity_at=$5 WHERE id=$6'
    });
    await backend.run(upd, status, owner, priority, updatedAt, updatedAt, patch.id);
  } else {
    const upd = sql({
...
export async function getTaskById(id) {
  return backend.get(
    sql({
      sqlite: 'SELECT id, status, owner, priority, updated_at FROM tasks WHERE id = ?',
      postgres: 'SELECT id, status, owner, priority, updated_at FROM tasks WHERE id = $1'
    }),
    id
  );
}
...
export async function countTasksByStatus(status, excludeId = null) {
  if (excludeId) {
    return Number(
      await backend.scalar(
        sql({
          sqlite: 'SELECT COUNT(*) FROM tasks WHERE status = ? AND id <> ?',
          postgres: 'SELECT COUNT(*) FROM tasks WHERE status = $1 AND id <> $2'
        }),
        status,
        excludeId
      )
  ...
  return Number(
    await backend.scalar(
      sql({
        sqlite: 'SELECT COUNT(*) FROM tasks WHERE status = ?',
        postgres: 'SELECT COUNT(*) FROM tasks WHERE status = $1'
      }),
      status
    )
  ) || 0;
...
export async function countTasksByOwnerAndStatus(owner, status, excludeId = null) {
  if (excludeId) {
    return Number(
      await backend.scalar(
        sql({
          sqlite: 'SELECT COUNT(*) FROM tasks WHERE owner = ? AND status = ? AND id <> ?',
          postgres: 'SELECT COUNT(*) FROM tasks WHERE owner = $1 AND status = $2 AND id <> $3'
        }),
        owner,
        status,
        excludeId
  ...
  return Number(
    await backend.scalar(
      sql({
        sqlite: 'SELECT COUNT(*) FROM tasks WHERE owner = ? AND status = ?',
        postgres: 'SELECT COUNT(*) FROM tasks WHERE owner = $1 AND status = $2'
      }),
      owner,
      status
    )
...
export async function addNote(taskId, note, actor) {
  ...
  if (await hasLastActivityColumn()) {
    await backend.run(
      sql({
        sqlite: 'UPDATE tasks SET updated_at=?, last_activity_at=? WHERE id=?',
        postgres: 'UPDATE tasks SET updated_at=$1, last_activity_at=$2 WHERE id=$3'
      }),
      ts,
      ts,
      taskId
...
export async function createHeartbeatRestartAttempt({ agentId, attemptedAt, reason, result, skipReason = null, cooldownUntil = null, attemptCountWindow = 0, idempotencyKey }) {
  const exists = await backend.get(
    sql({
      sqlite: 'SELECT id FROM heartbeat_restart_attempts WHERE idempotency_key = ?',
      postgres: 'SELECT id FROM heartbeat_restart_attempts WHERE idempotency_key = $1'
    }),
    idempotencyKey
  );
  ...
  await backend.run(
    sql({
      sqlite: 'INSERT INTO heartbeat_restart_attempts (agent_id, attempted_at, reason, result, skip_reason, cooldown_until, attempt_count_window, idempotency_key) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      postgres: 'INSERT INTO heartbeat_restart_attempts (agent_id, attempted_at, reason, result, skip_reason, cooldown_until, attempt_count_window, idempotency_key) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)'
    }),
    agentId,
    attemptedAt,
    reason,
...
export async function listHeartbeatRestartAttempts(agentId, sinceIso = null) {
  if (sinceIso) {
    return backend.all(
      sql({
        sqlite: 'SELECT agent_id as agentId, attempted_at as attemptedAt, reason, result, skip_reason as skipReason, cooldown_until as cooldownUntil, attempt_count_window as attemptCountWindow, idempotency_key as idempotencyKey FROM heartbeat_restart_attempts WHERE agent_id = ? AND attempted_at >= ? ORDER BY id DESC',
        postgres: 'SELECT agent_id as "agentId", attempted_at as "attemptedAt", reason, result, skip_reason as "skipReason", cooldown_until as "cooldownUntil", attempt_count_window as "attemptCountWindow", idempotency_key as "idempotencyKey" FROM heartbeat_restart_attempts WHERE agent_id = $1 AND attempted_at >= $2 ORDER BY id DESC'
      }),
      agentId,
      sinceIso
    );
  ...
  return backend.all(
    sql({
      sqlite: 'SELECT agent_id as agentId, attempted_at as attemptedAt, reason, result, skip_reason as skipReason, cooldown_until as cooldownUntil, attempt_count_window as attemptCountWindow, idempotency_key as idempotencyKey FROM heartbeat_restart_attempts WHERE agent_id = ? ORDER BY id DESC LIMIT 20',
      postgres: 'SELECT agent_id as "agentId", attempted_at as "attemptedAt", reason, result, skip_reason as "skipReason", cooldown_until as "cooldownUntil", attempt_count_window as "attemptCountWindow", idempotency_key as "idempotencyKey" FROM heartbeat_restart_attempts WHERE agent_id = $1 ORDER BY id DESC LIMIT 20'
    }),
    agentId
  );
}
...
export async function hasTaskRecoveryAction(idempotencyKey) {
  const row = await backend.get(
    sql({
      sqlite: 'SELECT id FROM task_recovery_actions WHERE idempotency_key = ?',
      postgres: 'SELECT id FROM task_recovery_actions WHERE idempotency_key = $1'
    }),
    idempotencyKey
  );
  return !!row;
...
export async function createTaskRecoveryAction({ taskId, agentId, result, idempotencyKey }) {
  ...
  await backend.run(
    sql({
      sqlite: 'INSERT INTO task_recovery_actions (task_id, agent_id, action_at, result, idempotency_key) VALUES (?, ?, ?, ?, ?)',
      postgres: 'INSERT INTO task_recovery_actions (task_id, agent_id, action_at, result, idempotency_key) VALUES ($1, $2, $3, $4, $5)'
    }),
    taskId,
    agentId,
    now(),
