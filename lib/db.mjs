...
async function ensureTenantFoundation() {
  const tenantTables = ['agents','tasks','task_notes','task_events','task_assignments','heartbeat_runs','heartbeat_restart_attempts','task_recovery_actions','standups','weekly_reports','tenant_agents','tenant_plans'];
  if (backend.mode === 'postgres') {
    ...
    await backend.exec('CREATE TABLE IF NOT EXISTS service_plans (id BIGSERIAL PRIMARY KEY, plan_key TEXT NOT NULL UNIQUE, name TEXT NOT NULL, description TEXT, monthly_price TEXT, default_team_template TEXT, max_agents INTEGER NOT NULL, max_wip INTEGER NOT NULL, max_tasks INTEGER, features_json TEXT, created_at TEXT NOT NULL)');
    await backend.exec('CREATE TABLE IF NOT EXISTS tenant_plans (id BIGSERIAL PRIMARY KEY, tenant_id TEXT NOT NULL UNIQUE, plan_key TEXT NOT NULL, status TEXT NOT NULL, activated_at TEXT, limits_json TEXT, created_at TEXT NOT NULL)');
    ...
    const plans = [
      { plan_key: 'starter', name: 'Starter', description: 'Small team', monthly_price: null, default_team_template: 'general_team', max_agents: 2, max_wip: 2, max_tasks: 200, features: { reports: true, restart: false } },
      { plan_key: 'professional', name: 'Professional', description: 'Balanced team', monthly_price: null, default_team_template: 'general_team', max_agents: 4, max_wip: 4, max_tasks: 1000, features: { reports: true, restart: true } },
      { plan_key: 'enterprise', name: 'Enterprise', description: 'Full team', monthly_price: null, default_team_template: 'general_team', max_agents: 8, max_wip: 8, max_tasks: 10000, features: { reports: true, restart: true, advanced_ops: true } }
    ];
    ...
    const generalTemplate = JSON.stringify({
      agents: seedTeam.map((agentId) => ({ agentId, role: 'agent', enabled: 1 })),
      defaults: { wip_limit_in_progress: 4, review_limit: 3 }
    });
    await backend.run('INSERT INTO team_templates (name, plan_key, template_json, created_at) VALUES ($1,$2,$3,$4) ON CONFLICT (name) DO NOTHING', 'general_team', 'professional', generalTemplate, now());
    return;
  }
  ...
  try { await backend.exec('CREATE TABLE IF NOT EXISTS service_plans (id INTEGER PRIMARY KEY AUTOINCREMENT, plan_key TEXT NOT NULL UNIQUE, name TEXT NOT NULL, description TEXT, monthly_price TEXT, default_team_template TEXT, max_agents INTEGER NOT NULL, max_wip INTEGER NOT NULL, max_tasks INTEGER, features_json TEXT, created_at TEXT NOT NULL)'); } catch {}
  try { await backend.exec('CREATE TABLE IF NOT EXISTS tenant_plans (id INTEGER PRIMARY KEY AUTOINCREMENT, tenant_id TEXT NOT NULL UNIQUE, plan_key TEXT NOT NULL, status TEXT NOT NULL, activated_at TEXT, limits_json TEXT, created_at TEXT NOT NULL)'); } catch {}
  ...
  for (const p of plans) {
    if (backend.mode === 'postgres') {
      ...
      await backend.run('INSERT OR IGNORE INTO service_plans (plan_key,name,description,monthly_price,default_team_template,max_agents,max_wip,max_tasks,features_json,created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', p.plan_key, p.name, p.description, p.monthly_price, p.default_team_template, p.max_agents, p.max_wip, p.max_tasks, JSON.stringify(p.features||{}), now());
    }
  }
  ...
  if (backend.mode === 'postgres') {
    ...
    await backend.run('INSERT OR IGNORE INTO team_templates (name, plan_key, template_json, created_at) VALUES (?, ?, ?, ?)', 'general_team', 'professional', generalTemplate, now());
  }
}
...
export async function listServicePlans() {
  return backend.all(
    sql({
      sqlite: 'SELECT plan_key as planKey, name, description, monthly_price as monthlyPrice, default_team_template as defaultTeamTemplate, max_agents as maxAgents, max_wip as maxWip, max_tasks as maxTasks, features_json as featuresJson, created_at as createdAt FROM service_plans ORDER BY id ASC',
      postgres: 'SELECT plan_key as "planKey", name, description, monthly_price as "monthlyPrice", default_team_template as "defaultTeamTemplate", max_agents as "maxAgents", max_wip as "maxWip", max_tasks as "maxTasks", features_json as "featuresJson", created_at as "createdAt" FROM service_plans ORDER BY id ASC'
    })
  );
}
...
export async function getServicePlan(planKey) {
  return backend.get(
    sql({
      sqlite: 'SELECT plan_key as planKey, name, description, monthly_price as monthlyPrice, default_team_template as defaultTeamTemplate, max_agents as maxAgents, max_wip as maxWip, max_tasks as maxTasks, features_json as featuresJson, created_at as createdAt FROM service_plans WHERE plan_key = ?',
      postgres: 'SELECT plan_key as "planKey", name, description, monthly_price as "monthlyPrice", default_team_template as "defaultTeamTemplate", max_agents as "maxAgents", max_wip as "maxWip", max_tasks as "maxTasks", features_json as "featuresJson", created_at as "createdAt" FROM service_plans WHERE plan_key = $1'
    }),
    planKey
  );
}

export async function setTenantPlan({ tenantId, planKey, status = 'active', activatedAt = null, limits = null }) {
  const limitsJson = JSON.stringify(limits || {});
  if (backend.mode === 'postgres') {
    await backend.run(
      'INSERT INTO tenant_plans (tenant_id, plan_key, status, activated_at, limits_json, created_at) VALUES ($1,$2,$3,$4,$5,$6) ON CONFLICT (tenant_id) DO UPDATE SET plan_key=EXCLUDED.plan_key, status=EXCLUDED.status, activated_at=EXCLUDED.activated_at, limits_json=EXCLUDED.limits_json',
      tenantId,
      planKey,
      status,
      activatedAt,
    ...
    await backend.run('INSERT OR REPLACE INTO tenant_plans (tenant_id, plan_key, status, activated_at, limits_json, created_at) VALUES (?, ?, ?, ?, ?, ?)', tenantId, planKey, status, activatedAt, limitsJson, now());
  }
}
...
export async function getTenantPlan(tenantId = TENANT_ID) {
  return backend.get(
    sql({
      sqlite: 'SELECT tenant_id as tenantId, plan_key as planKey, status, activated_at as activatedAt, limits_json as limitsJson, created_at as createdAt FROM tenant_plans WHERE tenant_id = ?',
      postgres: 'SELECT tenant_id as "tenantId", plan_key as "planKey", status, activated_at as "activatedAt", limits_json as "limitsJson", created_at as "createdAt" FROM tenant_plans WHERE tenant_id = $1'
    }),
    tenantId
  );
}
...
export async function getTeamTemplate(name = 'general_team') {
  return backend.get(
    sql({
      sqlite: 'SELECT name, template_json as templateJson, created_at as createdAt FROM team_templates WHERE name = ?',
      postgres: 'SELECT name, template_json as "templateJson", created_at as "createdAt" FROM team_templates WHERE name = $1'
    }),
    name
  );
}
