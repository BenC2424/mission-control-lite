...
async function ensureTenantFoundation() {
  const tenantTables = ['agents','tasks','task_notes','task_events','task_assignments','heartbeat_runs','heartbeat_restart_attempts','task_recovery_actions','standups','weekly_reports','tenant_agents'];
  if (backend.mode === 'postgres') {
  ...
  if (backend.mode === 'postgres') {
    await backend.run('INSERT INTO team_templates (name, template_json, created_at) VALUES ($1,$2,$3) ON CONFLICT (name) DO NOTHING', 'general_team', generalTemplate, now());
  } else {
    await backend.run('INSERT OR IGNORE INTO team_templates (name, template_json, created_at) VALUES (?, ?, ?)', 'general_team', generalTemplate, now());
  }
}
...
export async function createOnboardingRecord({ tenantId, requestedPlan = null, requestedTeamType = 'general_team', createdBy = 'system', notes = '' }) {
