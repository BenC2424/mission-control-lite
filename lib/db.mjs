    ...
    backend.get(
      sql({
        ...
        postgres: `SELECT
          ...
          COALESCE(SUM(CASE WHEN status = 'assigned' AND COALESCE(last_activity_at, updated_at::timestamptz, created_at::timestamptz) < $1::timestamptz THEN 1 ELSE 0 END),0)::int as assigned_gt_24h,
          COALESCE(SUM(CASE WHEN status = 'in_progress' AND COALESCE(last_activity_at, updated_at::timestamptz, created_at::timestamptz) < $2::timestamptz THEN 1 ELSE 0 END),0)::int as in_progress_gt_8h,
          COALESCE(SUM(CASE WHEN status = 'review' AND COALESCE(last_activity_at, updated_at::timestamptz, created_at::timestamptz) < $3::timestamptz THEN 1 ELSE 0 END),0)::int as review_gt_12h,
