...
export async function reassignTask(taskId, agentId) {
  ...
  await backend.run(
    sql({
      sqlite: 'UPDATE task_assignments SET completed_at = COALESCE(completed_at, ?) WHERE tenant_id = ? AND task_id = ? AND agent_id <> ? AND completed_at IS NULL',
      postgres: 'UPDATE task_assignments SET completed_at = COALESCE(completed_at, $1) WHERE tenant_id = $2 AND task_id = $3 AND agent_id <> $4 AND completed_at IS NULL'
    }),
    ts,
    TENANT_ID,
    taskId,
